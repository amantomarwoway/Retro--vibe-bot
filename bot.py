...
# MOVIE hai to 2-7 min cut, BHAJAN/SONG hai to pura
if os.path.exists("f.mp4"):
    ...
    if dur>420: # Badi movie
        cut=random.randint(120,420) # 2-7 min ke beech
    ...
    sp.call(["ffmpeg","-y","-loop","1","-i","https://i.imgur.com/8Km9tLL.jpg","-i","f.mp3","-c:v","libx264","-c:a","aac","-aspect","16:9","-shortest","c.mp4"],stdout=sp.DEVNULL,stderr=sp.DEVNULL)

# VIDEO TAB ME POST (Reel nahi)
tok=os.environ['FB_PAGE_TOKEN']
# Normal video feed ke liye title + description
